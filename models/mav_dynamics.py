...
class MavDynamics:
    ...
    def _f(self, state, forces_moments):
        ...
        # Position Kinematics
        #appendix B
        north_dot = u*(e0**2 + e1**2 - e2**2 - e3**2) +v*2*(e1*e2-e0*e3) +w*2*(e1*e3+e2*e0)
        east_dot  = u*2*(e1*e2+e0*e3) + v*(e0**2-e1**2+e2**2-e3**2) + w*2*(e2*e3-e0*e1)
        down_dot  = u*2*(e1*e3-e0*e2) + v*2*(e2*e3+e0*e1) + w*(e0**2-e1**2-e2**2+e3**2)

        # Position Dynamics
        u_dot = (r*v-q*w) +fx/MAV.mass #not m
